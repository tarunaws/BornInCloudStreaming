...
outputDirectoryMultipart = os.path.join("intermediate","split")
...
#Function :- Split
def split(jobId,splitContentId,bucketName,object_key,localPath):
...
#part of analyse
    ...
    #Conidering 1 GB copy time is 20 sec
    ...
    if int(fr_result) <= 25:
        ...
        actualFramePerSec = 25
    gopForOutput = int(fr_result) * gopFactor
    NoOfFramePerSegment = 3 * gopFactor * int(fr_result)
#    deliverySegSizeInMilisec = 1000 * float(NoOfFramePerSegment/actualFramePerSec) #Experimental if Packaging cause issue.
    deliverySegSizeInMilisec = 3 * 1000 * gopFactor
    transcodeDb.update_one(
                            {
                            "contentId":splitContentId
                            },
    ...
    command = f"ffmpeg -i {each} -c copy -acodec copy -segment_time {splitDuration} -f segment {outputname}%03d.{extension}"
